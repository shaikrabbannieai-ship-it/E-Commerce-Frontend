...
export default function PartnerPage() {
  ...
  const updateDeliveryLocation = async (lat: number, lng: number) => {
    try { await axios.post("http://127.0.0.1:8000/delivery/update-location", { partner_id: DELIVERY_PARTNER.id, lat, lng }); } catch (error) {}
  };

  const fetchAssignedOrders = async () => {
    setIsLoading(true);
    try {
      const response = await axios.get(`http://127.0.0.1:8000/orders/delivery/${DELIVERY_PARTNER.id}`);
  ...
  const updateOrderStatus = async (orderId: number, status: string) => {
    try { 
      await axios.put(`http://127.0.0.1:8000/order/update-status/${orderId}?status=${status}`); 
  ...
  const verifyOtpAndDeliver = async () => {
    ...
    try {
      ...
      const verifyResponse = await axios.post(`http://127.0.0.1:8000/order/verify-otp/${orderToDeliver?.id}`, {
        otp: enteredOtp
      });
      
      if (verifyResponse.data.valid) {
        // If OTP is valid, complete the delivery
        await axios.put(`http://127.0.0.1:8000/order/deliver/${orderToDeliver?.id}`);
  ...
  const markAsOutForDelivery = async (orderId: number) => {
    try {
      ...
      const response = await axios.post(`http://127.0.0.1:8000/order/out-for-delivery/${orderId}`);
